        ...
        <div className="card w-100 shadow-xss rounded-xxl border-0 mb-3 pr-2">
...

            {
                users && users.map((value, index) => (
                    <div className="wrap d-flex " key={index} style={{ alignSelf: 'start' }}>
                        <div className="card-body d-flex pt-0 ps-4 pe-4 pb-0 bor-0">
                            <figure className="avatar me-3"><img src={value.Avatar != null ? value.Avatar : "assets/images/user.png"} alt="avater" className="shadow-sm rounded-circle w45" /></figure>
                            <div>
                                <h4 style={{ textOverflow: "ellipsis", width: '100px', overflow: 'hidden',whiteSpace: 'nowrap' }} className="fw-700 text-grey-900 font-xssss mt-1 ">
                                    {value.username}
                                </h4>
                                <span className="d-block font-xssss fw-500 mt-1 lh-3 text-grey-500">{"10"} mutual friends</span>
                            </div>
